...
#   julia --project=docs/ docs/deploy_docs.jl v0.3.0    # specific tag → stable/ + vX.Y.Z/
#
...
repo_remote = "git@codeberg-docs:MicroPoroChemoMechanics/ChemistryLab.jl.git"
...
cd(pagesdir) do
    run(`git config user.name "ChemistryLab Documenter"`)
    run(`git config user.email "docs@chemistrylab.codeberg.page"`)
