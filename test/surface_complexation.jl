...
#
# Its provenance -- generator, versions, the md5 of phreeqc.dat, the site amount
# and the constants -- is in the fixture's own fields.
...
function _hfo_system(;
    ...
    support = SurfaceSupport("hydrous ferric oxide", nothing, FixedSurfaceArea(f.area))
    fam_s = SiteFamily(
...
@testsection "Surface complexation" begin
    ...
    @testset "residuals, one scale per conserved quantity" begin
        ...
        (; logK_protonation, logK_deprotonation, n_sites) = PHREEQC_PROTOLYSIS
        logK1, logK2 = logK_protonation, logK_deprotonation
        cs, st, _ = _amphoteric_system(; logK1, logK2, n_sites)
        des = DualEquilibriumSolver(cs, DiluteSolutionModel())
        A = Float64.(cs.SM.A)
        n0 = Float64[ustrip(us"mol", x) for x in st.n]
    ...
    @testset "a charged surface: the model, and where the solve stops" begin
        ...
        N, A = PHREEQC_HFO_ZN.n_weak, PHREEQC_HFO_ZN.area
        _sys(model) = begin
            cs, st, _ = _amphoteric_system(;
                logK1 = PHREEQC_PROTOLYSIS.logK_protonation,
                logK2 = PHREEQC_PROTOLYSIS.logK_deprotonation, n_sites = N,
            )
            fam = SiteFamily(
...
@testsection "the standard state of a free site is a gauge only while nothing follows" begin
    ...
    function run(Δ; coupled)
        ...
        mem = [
            ...
            surf("XwO-", g - RT25 * log(10.0^PHREEQC_PROTOLYSIS.logK_deprotonation)),
        ]
        support = SurfaceSupport(
