...
const RT_COUP = 8.31446261815324 * 298.15
...
function _sorbent_system(; logK_Ca = -3.0, n_sites = 2.0e-4)
    ...
    solid = Species("Ca(OH)2"; aggregate_state = AS_CRYSTAL, class = SC_COMPONENT)
    solid[:ΔₐG⁰] = _gc(-897010.0)

    free = sf("XsOH", 0.0)
    # XsOH + Ca+2 = XsOCa+ + H+, so ΔrG° = G(XsOCa+) − G(XsOH) − G(Ca+2) = −RT ln K
    bound = sf("XsOCa+", -RT_COUP * log(10.0^logK_Ca) - 552790.0)
...
@testsection "Surface complexation coupled to kinetics" begin
    ...
    @testset "the equilibrium partition keeps its site families" begin
        ...
        idx_eq = [i for (i, s) in enumerate(nm) if s != "Ca(OH)2"]
    ...
    @testset "a constant-rate release onto an inert sorbent" begin
        ...
        k_rate = 2.0e-8                      # mol/s
        rxn = Reaction(
            OrderedDict(spc["Ca(OH)2"] => 1),
            OrderedDict(spc["Ca+2"] => 1, spc["OH-"] => 2);
            symbol = "dissolution", equal_sign = '→',
        )
        st = zeros(Float64, length(nm))
        st[idx("Ca(OH)2")] = -1.0; st[idx("Ca+2")] = 1.0; st[idx("OH-")] = 2.0
        kr = KineticReaction(rxn, (T, P, t, n, lna, n0) -> k_rate, idx("Ca(OH)2"), st)

        n0 = Any[fill(0.0u"mol", length(nm))...]
        n0[idx("H2O@")] = 55.5u"mol"
        n0[idx("Ca(OH)2")] = 1.0e-3u"mol"
        ...
        last = states[end]
        solid_left = ustrip(us"mol", last.n[idx("Ca(OH)2")])
