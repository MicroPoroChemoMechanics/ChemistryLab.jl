...
@testsection "which parameters a measurement can determine" begin
    ...
    @testset "a fitted number is not a measured one" begin
        ...
        @test provenance(out[1]) === PROV_FITTED
        @test provenance(out[2]) === PROV_FITTED
        # AND THE ONE THE DATA DID NOT CONSTRAIN IS NOT REPORTED AS FITTED. It
        # is a value the optimization had to put somewhere, which is what a
        # placeholder is.
        ...
        all_fitted = as_traced(id, _θ; source = "a synthetic fit", rank_limit = false)
